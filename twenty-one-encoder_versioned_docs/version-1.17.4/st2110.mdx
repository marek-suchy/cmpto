...
If you wish for NMOS configuration to be made available on a different network interface, please use the Setup dialogue. In the Network connectivity step, select the desired network interface and check Administration + NMOS checkbox. You can use multiple network interfaces with NMOS configuration enabled. Note that when NMOS configuration is enabled on a network interface, the discovery of NMOS registries on the interface starts automatically.
...
ST 2110-20 Output Stream Specification
...
ST 2110-30 Output Stream Specification
